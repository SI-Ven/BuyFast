...
public interface ProductRepo {
    ...
    @Select("SELECT p.id, p.product_uuid as productUuid, p.product_name as productName, " +
            "p.description, p.is_active as active, " +
            "b.brand_name as brandName, " +
            "c.category_name as categoryName, " +
            "mc.main_category_name as mainCategoryName, " +
            "(SELECT image_url FROM product_image WHERE product_id = p.id LIMIT 1) as mainImage " +
            "FROM product p " +
